...
def test_expect_to_be_visible_timeout_reload(page: Page):
    # Passes → Handles intermittent behaviour with reloads every time
...
def test_expect_to_be_visible_timeout_reload_optimal(page: Page):
    # Passes → Handles intermittent behaviour with reloads after first try
    ...
    def check():
        try:
            expect(button_locator).to_be_visible()
            return True  # or use a custom check_success
        except AssertionError:
            page.reload()
            return False  # or use a custom check_success
